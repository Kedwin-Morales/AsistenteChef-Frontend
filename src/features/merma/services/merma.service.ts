...
import type { ModelDTO, CreateDTO } from "../types/merma.types";
...
export const getAll = async () => {
  const { data } = await api.get("/Merma/lista");
  return data;
};

export const getById = async (id: string) => {
  const { data } = await api.get(`/Merma/Buscar/${id}`);
  return data as ModelDTO;
};

export const crear = async (dto: CreateDTO) => {
  const formData = toFormData(dto);
  const res = await api.post("/Merma/Crear", formData);
  return res.data;
};

export const editar = async (id: string, dto: CreateDTO) => {
  const formData = toFormData(dto);
  const res = await api.put(`/Merma/Editar/${id}`, formData);
  return res.data;
};

export const anular = async (id: string, data: ModelDTO) => {
  data.mermaId = id;
  const res = await api.put(`/Merma/anular/${id}`, data);
  return res.data;
};

export const eliminar = async (id: string) => {
  const res = await api.delete(`/Merma/Eliminar/${id}`);
  return res.data;
};