...
import { useFamiliaMenu } from "../hooks/useFamiliaMenu";
import { crear, editar } from "../services/familiaMenu.service";
import type { ModelDTO } from "../types/familiaMenu.types";
...
export default function FamiliaMenuPage() {
  const { familias, loading, refetch } = useFamiliaMenu();
  ...
  const consejo = consejos.filter((a)=> a.modulo === "Familias del Menú".toUpperCase() && a.activo );
  ...
  const handleSubmit = async (data: Partial<ModelDTO>) => {
    ...
    try {
      ...
      if (modalMode === "edit" && selected?.familiaMenuId) {
        await editar(selected.familiaMenuId, {
          familiaMenuId: "",
          nombre: data.nombre,
          descripcion: data.descripcion ?? "",
  ...
  const confirmarDelete = async (item: ModelDTO) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await editar(item.familiaMenuId, {
    ...
    <AppLayout>
      ...
      <div className="my-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard
          ...
          icon={NotebookPen}
          value={familias.filter((a) => a.activo).length}
      ...
      <EntityModal<ModelDTO>
        open={modalOpen}
        key={`${modalMode}-${selected?.familiaMenuId ?? "new"}`}
        title={
          modalMode === "create"
            ? "Crear"
            : modalMode === "edit"
